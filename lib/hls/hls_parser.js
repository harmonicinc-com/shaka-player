...
shaka.hls.HlsParser = class {
  ...
  constructor() {
    ...
    /** A map that stores the EXT-X-DATERANGE tags already parsed.
     * The key is the tag's ID value.
     * @private {!Map.<string, !shaka.extern.TimelineRegionInfo>}
     */
    this.regionIdToRegionInfosMap_ = new Map();
  }
  ...
  stop() {
    ...
    this.regionIdToRegionInfosMap_.clear();
      ...
      if (method != 'NONE') {
        ...
        if (method == 'AES-128') {
          ...
          if (drmInfo) {
            ...
            if (drmInfo.keyUri && drmInfo.keyIds && drmInfo.keyIds.size > 0) {
              // Get key using the key URI if provided & valid
              drmInfo.fetchKey = async (currentDrm) => {
                const requestType = shaka.net.NetworkingEngine.RequestType.KEY;
                const request = shaka.net.NetworkingEngine.makeRequest(
                    [drmInfo.keyUri], this.config_.retryParameters);
                const keyResponse =
                ...
                const kidUint8 = shaka.util.Uint8ArrayUtils
                    .fromHex([...drmInfo.keyIds || ''][0]);
                const license = JSON.stringify({
                  keys: [{
                    kty: 'oct',
                    kid: shaka.util.Uint8ArrayUtils
                        .toBase64(kidUint8, false),
                    k: shaka.util.Uint8ArrayUtils
                        .toBase64(keyResponse.data, false),
                  }],
                });
                currentDrm.licenseServerUri = 'data:application/json;base64,' +
                  window.btoa(license);
                currentDrm.fetchKey = undefined;
              };
            }
  ...
  createSegments_(verbatimMediaPlaylistUri, playlist, type, mimeType,
    ...
    // Calculate the offset time for EXT-X-DATERANGE tags to use
    const firstProgramDateTimeTag = shaka.hls.Utils.getFirstTagWithName(
        playlist.segments[0].tags, 'EXT-X-PROGRAM-DATE-TIME');
    const offsetTime = this.calculateOffsetTime_(firstStartTime,
        firstProgramDateTimeTag);

    // Remove from the map if the region's endTime is before the DVR window
    for (const regionInfo of this.regionIdToRegionInfosMap_.values()) {
      if (regionInfo.endTime < firstStartTime) {
        this.regionIdToRegionInfosMap_.delete(regionInfo.id);
      }
    }
    ...
    for (let i = 0; i < hlsSegments.length; i++) {
      ...
      const segmentDateRangeTags = shaka.hls.Utils.filterTagsByName(
          item.tags, 'EXT-X-DATERANGE');

      for (const dateRangeTag of segmentDateRangeTags) {
        const newRegion = this.parseDateRangeTag_(dateRangeTag, offsetTime);

        if (newRegion.id && this.regionIdToRegionInfosMap_.has(newRegion.id)) {
          const existingRegion = this.regionIdToRegionInfosMap_
              .get(newRegion.id);

          if (existingRegion.endTime !== newRegion.endTime) {
            existingRegion.endTime = newRegion.endTime;

            const existingDateRange = existingRegion.hlsDateRangeInfo;
            const newDateRange = newRegion.hlsDateRangeInfo;

            if (existingDateRange && newDateRange) {
              // The DURATION tag should be final, if it was not set before
              // and it is set now, update the region's duration
              if (newDateRange.duration) {
                existingDateRange.duration = newDateRange.duration;
              } else if (newDateRange.plannedDuration) {
                existingDateRange.plannedDuration = newDateRange
                    .plannedDuration;
              }
            }

            // Update this region because the endTime (duration) has changed
            this.playerInterface_.onTimelineRegionAdded(existingRegion);
          }
        } else {
          this.playerInterface_.onTimelineRegionAdded(newRegion);
          this.regionIdToRegionInfosMap_.set(newRegion.id, newRegion);
        }
      }
    }
  ...
  /**
   * Calculates the offset time in seconds relative to the first start time of
   * the video.
   *
   * For example, if the first start time is 100 and the epoch second of the
   * first segment in the current playlist is 1696932610 (which is
   * 2023-10-10T10:10:10Z), then it will return (1696932610 - 100 = 1696932510).
   *
   * This value can then be used to subtract from the epoch seconds of a
   * datetime so that the video time of the datetime can be calculated.
   *
   * @param {number} firstStartTime
   * @param {?shaka.hls.Tag} firstProgramDateTimeTag
   * @return {number}
   * @private
   */
  calculateOffsetTime_(firstStartTime, firstProgramDateTimeTag) {
    let offsetTime = -firstStartTime;
    if (firstProgramDateTimeTag) {
      const firstDateTimeValue = firstProgramDateTimeTag.value;
      offsetTime += new Date(firstDateTimeValue).getTime() / 1000;
    }
    return offsetTime;
  }

  /**
   * Parses an EXT-X-DATERANGE tag.
   *
   * The offsetTime is the epoch seconds to subtract from the START-DATE and
   * END-DATE such that the time values are relative to the video time.
   *
   * For example, if the offsetTime is 1696932510 (from the example in
   * calculateOffsetTime_) and the START-DATE is 2023-10-10T10:11:00Z
   * (1696932660), then the adjusted startTime will become 150.
   *
   * @param {!shaka.hls.Tag} tag
   ...
   * @private
   */
  parseDateRangeTag_(tag, offsetTime) {
    const startDateValue = tag.getRequiredAttrValue('START-DATE');
    const startTime = new Date(startDateValue).getTime() / 1000 -
          (offsetTime || 0);

    const endDateValue = tag.getAttributeValue('END-DATE');
    let endTime = new Date(endDateValue).getTime() / 1000 - (offsetTime || 0);

    const durationValue = tag.getAttributeValue('DURATION');
    const plannedDurationValue = tag.getAttributeValue('PLANNED-DURATION');

    // if END-DATE is not available, set it to START-DATE + DURATION
    // if DURATION is not available, use PLANNED-DURATION
    if (!endDateValue) {
      if (durationValue) {
        endTime = startTime + Number(durationValue);
      } else {
        endTime = startTime + Number(plannedDurationValue);
      }
    }
    ...
    for (const attribute of tag.attributes) {
      ...
      if (name.startsWith('X-')) {
        customAttributes[name] = attribute.value;
      }
    }

    /** @type {shaka.extern.HlsDateRangeInfo} */
    const dateRange = {
      class: tag.getAttributeValue('CLASS'),
      duration: Number(durationValue),
      plannedDuration: Number(plannedDurationValue),
  ...
  static identityDrmParser_(drmTag) {
    ...
    // Use KEYID if available
    const keyIdHex = drmTag.getAttributeValue('KEYID');
    const keyUri = drmTag.getAttributeValue('URI');
    if (keyIdHex && keyUri && shaka.hls.HlsParser.isValidHttpUrl(keyUri)) {
      // Common PSSH
      const keyId = keyIdHex.toLowerCase().substring(2);

      goog.asserts.assert(keyId, 'Key ID must ' +
        'not be empty');

      const initDataStr = JSON.stringify({'kids': [keyId]});
      const initData = shaka.util.BufferUtils.toUint8(
          shaka.util.StringUtils.toUTF8(initDataStr));
      const initDatas = {
        initDataType: 'keyids',
        initData: initData,
      };
  ...
  /**
   * URL validator
   * @param {string|null} str URL to be validated
