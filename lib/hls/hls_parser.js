  ...
  createSegments_(verbatimMediaPlaylistUri, playlist, type, mimeType,
    ...
    for (let i = 0; i < hlsSegments.length; i++) {
      ...
      for (const dateRangeTag of segmentDateRangeTags) {
        const region = this.parseDateRangeTag_(dateRangeTag, offsetTime);

        if (region.id && this.regionIdToRegionInfosMap_.has(region.id)) {
          const existingRegion = this.regionIdToRegionInfosMap_.get(region.id);
          if (existingRegion.endTime === region.endTime) {
          // The duration of the region is unchanged, no need to update.
            continue;
          }
        }
        this.playerInterface_.onTimelineRegionAdded(region);
        this.regionIdToRegionInfosMap_.set(region.id, region);
      }
    }
