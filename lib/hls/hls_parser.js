...
shaka.hls.HlsParser = class {
    ...
    if (playlist.segments) {
      for (const segment of playlist.segments) {
        const segmentDateRangeTags = shaka.hls.Utils.filterTagsByName(
            segment.tags, 'EXT-X-DATERANGE');
        dateRangeTags.push(...segmentDateRangeTags);
      }
    }

    const syncTime = segmentIndex.earliestReference().syncTime;
    for (const dateRangeTag of dateRangeTags) {
      const region = this.parseDateRangeTag_(dateRangeTag, syncTime);
   ...
   * @private
   */
  parseDateRangeTag_(tag, syncTime) {
    ...
    let endTime = new Date(endDateValue).getTime() / 1000 - (syncTime || 0);
