...
shaka.dash.DashParser = class {
  ...
  async processManifest_(mpd, finalManifestUri) {
    ...
    if (patchLocation.length > 0) {
      ...
      if (index > 0) {
        this.patchLocation_ = this.finalManifestUri_.substr(0, index + 1) + patchLocation;
      }
    }
   ...
   * @private
   */
  async parseManifestPatch_(data) {
    const Error = shaka.util.Error;
    let text = shaka.dash.DashParser.replaceNamespace_(
  ...
  static replaceNamespace_(text, namespace) {
    ...
    if (startIndex > 0) {
      ...
      if (endIndex > 0) {
        return text.substr(0, startIndex + prefix.length) + namespace + text.substr(endIndex);
      }
    }
    return text;
  }
   ...
   * @private
   */
  getNodeByXPath_(evaluator, mpd, xpath) {
    ...
    if (!result || !result.singleNodeValue) return null;
  ...
  patchAdd_(evaluator, mpd, operation) {
    ...
    const node = this.getNodeByXPath_(evaluator, mpd, xpath);
    if (!node) {
      return false;
    }
  ...
  patchReplace_(evaluator, mpd, operation) {
    let xpath = operation.getAttribute('sel');
    const lastSlash = xpath.lastIndexOf('/');
    const lastElement = xpath.substr(lastSlash + 1);
    if (lastElement.lastIndexOf('@') >= 0) {
      // Replace attribute
      const node = this.getNodeByXPath_(evaluator, mpd, xpath.substr(0, lastSlash));
      if (!node) {
        return false;
      }
