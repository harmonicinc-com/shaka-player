...
shaka.dash.DashParser = class {
  ...
  async update() {
    try {
      if (this.patchLocation_) {
        ...
        await this.requestManifest_(true);
      }
    } catch (error) {
   ...
   * @private
   */
  async requestManifest_(force = false) {
    ...
    await this.parseManifest_(response.data, response.uri, force);
   ...
   * @private
   */
  async parseManifest_(data, finalManifestUri, force = false) {
    ...
    if (disableXlinkProcessing) {
      return this.processManifest_(mpd, finalManifestUri, force);
    }
    ...
    return this.processManifest_(finalMpd, finalManifestUri, force);
  }
   ...
   * @private
   */
  async processManifest_(mpd, finalManifestUri, force = false) {
    ...
    if (force) {
      shaka.log.debug('Wiping periods and streamMap');
      this.periodCombiner_.release();
      this.streamMap_ = {};
    }
    ...
    if (force && this.playerInterface_.abrManager) {
      this.playerInterface_.abrManager.setVariants(
          this.periodCombiner_.getVariants());
    }
    ...
    if (!this.manifest_) {
      ...
      this.manifest_.presentationStartTime = presentationStartTime;
