...
shaka.dash.DashParser = class {
      ...
      for (const producerReferenceTime of producerReferenceTimes) {
        ...
        for (const stream of streams) {
          ...
          const data = {
            ...
            pta: pta,
            pto: pto,
            periodStart: context.periodInfo.start,
            isInBand: isInBand,
            timescale: ts,
            type: type,
          };
  ...
  async requestForTiming_(baseUris, uri, method) {
    ...
    // Check ISO date precision
    // eslint-disable-next-line max-len
    const isoFormatMatch = text.match(/(\d{4})-(\d\d)-(\d\d)T(\d\d):(\d\d)(?::(\d\d)(?:\.(\d\d\d))?)?/);
    const hasMs = isoFormatMatch ? isoFormatMatch[7] : true;
    if (!hasMs) {
      this.playerInterface_.onEvent(
          new shaka.util.FakeEvent('imprecisetime', null),
      );
    }
      ...
      if (child.nodeName === 'Period') {
        ...
        while (innerChild) {
          if (innerChild.nodeName === 'AdaptationSet') {
            // This is the count of segmentIndex.references
            // Since segmentIndex will evaluate it's only eviction,
            // to maintains it only contains segments required to fit into DVR window.
            // So it's NOT segmentCount of full MPD, NEITHER equivalent to unfolded this.mpd_.
            const segmentCount = this.getSegmentCountFromSegmentIndex_(
                innerChild, periodId);
            removeCount += this.evictSegmentsFromAdaptationSet_(innerChild,
                segmentCount, isFirstPeriod);
  ...
  getSegmentCountFromSegmentIndex_(node, periodId) {
    ...
    if (representations && representations.length > 0) {
      ...
      const segmentIndex = this.streamMap_[id].segmentIndex;
      if (segmentIndex) {
    ...
    if (segmentTemplate && segmentTemplate.length > 0) {
      ...
      if (segmentTimeline && segmentTimeline.length > 0) {
        const removeCount =
            ...
            segmentCount;
        // This is special handle for PMM DASH w/mpdpatch
        // If only one <S> presents in first period, don't evict.
        if (isFirstPeriod) {
          const segments = segmentTimeline[0].getElementsByTagName('S');
          if (segments.length == 1) return 0;
          console.log(`segmentTemplate[0].getAttribute("availabilityTimeComplete") = ${segmentTemplate[0].getAttribute("availabilityTimeComplete")}`);
          if (segments.length == 2 && segmentTemplate[0].getAttribute("availabilityTimeComplete") == "false") return 0;
        }
  ...
  patchReplace_(evaluator, mpd, operation) {
    ...
    if (lastElement.startsWith('@')) {
      ...
      node.parentElement.replaceChild(operation.firstElementChild, node);
    }
    return true;
  }
