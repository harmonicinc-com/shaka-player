...
shaka.text.VttTextParser = class {
  ...
  static parseCueStyles(payload, rootCue) {
    payload = shaka.text.VttTextParser.replaceClassPayload(payload);
  ...
  /**
   * Parses a WebVTT styles from the given payload.
   * For example, <c.yellow.bg_blue>Yellow text on blue background</c>
   * https://w3c.github.io/webvtt/#default-text-color
   *
   * @param {string} payload
   * @return {string} processed payload
   */
  static replaceClassPayload(payload) {
      ...
      for (const tag of tags) {
        switch (tag) {
          ...
          case 'u':
            ...
            break;
          default:
            if (textColor.hasOwnProperty(tag)) {
              nestedCue.color = textColor[tag];
            } else if (bgColor.hasOwnProperty(tag)) {
