...
/**
 * @event shaka.Player.DownloadCompleteEvent
 * @description Fired when download manifest/segment completed
 * @property {string} type
 *    'downloadcomplete'
 * @property {number} downloadType
 *    shaka.Player.DownloadType
 * @property {BufferSource} data
 *    The buffer of downloaded data
 * @property {string} uri
 *    The uri of downloaded content
 * @exportDoc
 */
...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  async onLoad_(has, wants) {
      ...
      if (this.abrManager_ instanceof shaka.abr.SimpleLLAbrManager) {
        const playerInterface = {
          ...
          getLastSegmentRef: (mediaType) =>
            this.streamingEngine_.getLastSegmentRef(mediaType),
          getLiveEdge: () =>
    ...
    // If start time is not provided and ServiceDescription exists
    // Offset start time by ServiceDescription.latency.target
    if (!has.startTime && this.parser_ instanceof shaka.dash.DashParser) {
      const serviceDescription = this.parser_.getServiceDescription();
      if (serviceDescription &&
          serviceDescription.latency && serviceDescription.latency.target) {
        has.startTime = serviceDescription.latency.target / -1000;
      }
    }
  ...
  createNetworkingEngine() {
    ...
    const onProgressUpdated_ = (deltaTimeMs, bytesDownloaded, uri) => {
      ...
      if (this.abrManager_) {
        this.abrManager_.segmentDownloaded(deltaTimeMs, bytesDownloaded, uri);
      }
    };
   ...
   */
  onSegmentDownloadCompleted(deltaTimeMs, data, uri, isFullSpeed) {
    if (this.abrManager_) {
      this.abrManager_.segmentDownloadCompleted(deltaTimeMs, data.byteLength,
          [uri], isFullSpeed);
    }

    const eventName = shaka.Player.EventName.DownloadComplete;
    let downloadType = shaka.Player.DownloadType.UNKNOWN;
    const activeTrack = this.getActiveTrack();
    if (uri.includes(activeTrack.originalAudioId)) {
      downloadType = shaka.Player.DownloadType.AUDIO_SEGMENT;
      const mediaState = this.getMediaState('audio');
      const ref = mediaState.lastInitSegmentReference;
      if (ref && ref.getUris().includes(uri)) {
          downloadType = shaka.Player.DownloadType.AUDIO_INIT_SEGMENT;
      }
    } else if (uri.includes(activeTrack.originalVideoId)) {
      downloadType = shaka.Player.DownloadType.VIDEO_SEGMENT;
      const mediaState = this.getMediaState('video');
      const ref = mediaState.lastInitSegmentReference;
      if (ref && ref.getUris().includes(uri)) {
        downloadType = shaka.Player.DownloadType.VIDEO_INIT_SEGMENT;
      }
    }
    ...
    this.dispatchEvent(this.makeEvent_(eventName, eventData));
  }

  /**
   * Returns current active track
  ...
  createStreamingEngine() {
    ...
    const playerInterface = {
      ...
      onSegmentDownloadCompleted: (durationMs, data, uris, isFullSpeed) =>
        this.onSegmentDownloadCompleted(durationMs, data, uris,
            isFullSpeed),
    };
 ...
 */
 shaka.Player.DownloadType = {
  'UNKNOWN': 0,
  'MANIFEST': 1,
  'AUDIO_INIT_SEGMENT': 2,
  'AUDIO_SEGMENT': 3,
  'VIDEO_INIT_SEGMENT': 4,
  'VIDEO_SEGMENT': 5,
};
