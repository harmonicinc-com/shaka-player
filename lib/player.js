...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  load(assetUri, startTime, mimeType) {
    return this.loadWithOffset(assetUri, startTime, mimeType, null);
  }

  /**
   * Same as |load|, but allows the caller to specify start time offset
   * @param {string} assetUri
   * @param {?number=} startTime
   * @param {string=} mimeType
   ...
   * @export
   */
  loadWithOffset(assetUri, startTime, mimeType, startTimeOffset) {
    // Do not allow the player to be used after |destroy| is called.
    if (this.loadMode_ == shaka.Player.LoadMode.DESTROYED) {
      return Promise.reject(this.createAbortLoadError_());
    }
    ...
    if (startTimeOffset !== undefined) {
      payload.startTimeOffsetMs = startTimeOffset;
    }
  ...
  async onLoad_(has, wants) {
    ...
    if (this.isLive() &&
      wants.startTimeOffsetMs !== undefined &&
      wants.startTimeOffsetMs !== null) {
      has.startTime = this.manifest_.presentationTimeline
          .getSegmentAvailabilityEnd() - wants.startTimeOffsetMs;
  ...
  getProducerReferenceTimeInfo() {
    return (this.isLive() && this.streamingEngine_) ?
