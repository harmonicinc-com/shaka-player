...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  async onLoad_(has, wants) {
    ...
    // Apply presentation delay to avoid buffering on playback start
    const startDelay = this.manifest_.presentationTimeline.isLive() ?
        this.manifest_.presentationTimeline.getDelay() : 0;
    this.playhead_ = this.createPlayhead(has.startTime - startDelay);
  ...
  getPresentationLatency() {
    if (this.assetUri_ == null) {
      shaka.log.debug('getPresentationLatency this.assetUri not available yet');
      return null;
    }
    if (!this.assetUri_.endsWith('.mpd')) {
      shaka.log.debug('getPresentationLatency asset is not dash.');
      return null;
    }
    if (!(this.parser_ instanceof shaka.dash.DashParser)) {
      shaka.log.debug('getPresentationLatency parser is not dash parser.');
      return null;
    }
    const data = this.parser_.getPresentationLatencyData();
    const presentationLatencyArray = this.streamingEngine_
        .getPresentationLatency(new Date().getTime(),
            this.playhead_.getTime()*1000, data);

    return presentationLatencyArray;
  }
