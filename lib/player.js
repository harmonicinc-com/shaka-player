...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  async onLoad_(has, wants) {
    ...
    if (!this.abrManager_ || this.abrManagerFactory_ != abrFactory) {
      ...
      if (this.abrManager_ instanceof shaka.abr.SimpleLLAbrManager) {
        const playerInterface = {
          getPresentationLatencyInfo: () => this.getPresentationLatencyInfo(),
    ...
    if (this.isLive() &&
      this.config_.streaming.liveCatchUp.targetLiveLatencyOverride > 0) {
      // If the user has overridden the target live latency, then set the
      // playhead to (liveEdge - targetLiveLatencyOverride).
      has.startTime = this.manifest_.presentationTimeline
          .getSegmentAvailabilityEnd() - (this.config_.streaming.liveCatchUp
          .targetLiveLatencyOverride / 1000);
    } else if (!has.startTime &&
      this.parser_ instanceof shaka.dash.DashParser) {
   ...
   */
  onSegmentDownloadCompleted(deltaTimeMs, numBytes, uris, isFullSpeed) {
    if (this.abrManager_) {
      this.abrManager_.segmentDownloadCompleted(deltaTimeMs, numBytes,
          uris, isFullSpeed);
    }
  }
  ...
  createStreamingEngine() {
    ...
    const playerInterface = {
      ...
      onSegmentDownloadCompleted: (durationMs, numBytes, uris, isFullSpeed) =>
        this.onSegmentDownloadCompleted(durationMs, numBytes, uris,
            isFullSpeed),
      abrManager: this.abrManager_,
      createPlayhead: (startTime) => this.createPlayhead(startTime),
    };
  ...
  /**
   * Return a list of image streams.
   *
   * If the player has not loaded content, this will return an empty list.
   *
   ...
   */
  getImageStreams() {
    return this.manifest_ ? this.manifest_.imageStreams : [];
  }
  ...
  /**
   * Get producer reference time info (PRFT)
   *
   * @return {
  ...
  getProducerReferenceTimeInfo() {
    return this.streamingEngine_ ?
      this.streamingEngine_.getProducerReferenceTimeInfo() : null;
  }

  /**
   * Get presentation latency information
   * Both encoder and captured latency will be returned
   *
   * Used in Web Player SDK - Debug overlay
   *
  ...
  getPresentationLatencyInfo() {
    return this.streamingEngine_ ?
      this.streamingEngine_.getPresentationLatencyInfo() : null;
  }
