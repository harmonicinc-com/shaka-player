...
shaka.net.HttpFetchPlugin = class {
  ...
  static parse(uri, request, requestType, progressUpdated) {
    ...
    const pendingRequest = shaka.net.HttpFetchPlugin.request_(
        uri, requestType, init, abortStatus, request, progressUpdated);
   ...
   * @private
   */
  static async request_(uri, requestType, init, abortStatus, request,
      progressUpdated) {
    ...
    try {
      ...
      let segmentAppender = null;
      if (request.customObj != null) {
        if (request.customObj.constructor === shaka.media.SegmentAppender) {
          segmentAppender = request.customObj;
        }
      }
      ...
      const start = (controller) => {
        const push = async () => {
          ...
          if (segmentAppender && response.status === 200) {
            await segmentAppender.onReadUpdate(readObj);
          }
