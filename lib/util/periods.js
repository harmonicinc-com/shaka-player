...
shaka.util.PeriodCombiner = class {
  ...
  constructor() {
    ...
    /**
     * Map of IDs and start time of the periods we have already used to generate
     * streams.
    ...
    if (!isDynamic && periods.length == 1) {
      ...
      for (let i = 0; i < periods.length; i++) {
        ...
        if (this.usedPeriodIds_.has(period.id)) {
          const oldStartTime = this.usedPeriodIds_.get(period.id);
          if (oldStartTime !== period.start) {
            // Period out-of-sync. Considered as new.
            shaka.log.debug('Period out of sync. Drop all old periods');
            ...
            this.usedPeriodIds_.set(period.id, period.start);
            firstNewPeriodIndex = i;
            break;
          }
          // Else do nothing. This isn't new.
        } else {
          // This one _is_ new.
          this.usedPeriodIds_.set(period.id, period.start);
  ...
  static async extendOutputSegmentIndex_(outputStream, firstNewPeriodIndex) {
    ...
    if (outputStream.segmentIndex instanceof shaka.media.MetaSegmentIndex) {
      for (let i = 0; i < streams.length; i++) {
        ...
        console.log(match.segmentIndex);
    ...
    // Scan the indices. Fix when needed
    outputStream.segmentIndex.dropStaleReferences();
  }
