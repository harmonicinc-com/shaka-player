...
shaka.abr.SimpleLLAbrManager = class {
  ...
  constructor() {
    ...
    /** @private {number} */
    this.lowBufferLevelCount_ = 0;

    /** @private {number} */
    this.maxLowBufferLevelCount_ = 2;
  ...
  monitorBuffer_() {
    ...
    } else if (this.bufferLevelHistory_.length < this.maxBufferHistoryCount_) {
    ...
    if (this.bufferLevelHistory_.length == this.maxBufferHistoryCount_) {
      ...
      if (bufferLevel < average) {
        ...
        if (factor > this.bufferLevelDropToDecreaseBitrate_) {
          isLower = true;
          this.lowBufferLevelCount_++;
          if (this.lowBufferLevelCount_ > this.maxLowBufferLevelCount_) {
            this.decreaseVideoBitrate_();
            this.lowBufferLevelCount_ = 0;
          }
        }
