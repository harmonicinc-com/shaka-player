  ...
  increaseVideoBitrate_() {
    ...
    if (this.enabled_) {
      ...
      const presentationLatency =
          this.playerInterface_.getProducerReferenceTimeInfo();
      let latency = 0;
      const pt = this.playerInterface_.getPresentationTime() * 1000;
      const usePrft = presentationLatency[1] &&
        presentationLatency[1]['type'] === 'prft';
      if (serviceDescription) {
        if (usePrft) {
          // MP4 PRFT mode
          // Use encoder WCA/PTA when possible
          const info = presentationLatency[1];
          const wca = info['wca'][1] !== undefined ?
            info['wca'][1] : info['wca'][0];
          const pta = info['pta'][1] !== undefined ?
            info['pta'][1] : info['pta'][0];
          const wc = wca + (pt - pta);
          latency = Date.now() - wc;
        } else {
          // MPD mode
          const info = presentationLatency[0];
          latency = ((info['wc'] - info['wca'][1]) - (pt - info['pta'][1]));
        }
      }
 ...
 *   getProducerReferenceTimeInfo: function():Array<Object>,
 ...
 * @property {function():Array<Object>} getProducerReferenceTimeInfo
