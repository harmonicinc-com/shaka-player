...
shaka.media.GapJumpingController = class {
  ...
  onPollGapJump_() {
    ...
    let eventMessage;
    if (gapIndex == 0) {
      eventMessage = `Jumping forward ${jumpSize}s because of gap before start
       time of ${jumpTo}`;
      shaka.log.info(
          'Jumping forward', jumpSize,
          'seconds because of gap before start time of', jumpTo);
    } else {
      eventMessage = `Jumping forward ${jumpSize}s because of gap starting at 
      ${buffered.end(gapIndex - 1)} and ending at ${jumpTo}`;
      shaka.log.info(
          'Jumping forward', jumpSize, 'seconds because of gap starting at',
          buffered.end(gapIndex - 1), 'and ending at', jumpTo);
    }
    this.video_.dispatchEvent(
        new CustomEvent('gapjump', {detail: eventMessage}));
