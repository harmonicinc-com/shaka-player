...
shaka.media.RegionTimeline = class extends shaka.util.FakeEventTarget {
  ...
  filterBySeekRange_() {
    ...
    for (const region of this.regions_) {
      ...
      //
      // Add 1 second buffer to prevent removing region before observers are
      // notified of when the region is exited.
      if (region.endTime + shaka.media.RegionTimeline.DELETE_REGION_BUFFER <
        seekRange.start) {
...
/** @const {number} */
shaka.media.RegionTimeline.DELETE_REGION_BUFFER = 1; // in seconds
