...
shaka.media.SegmentAppender = class {
  ...
  */
  onSegmentBoxReady(data) {
    const hasClosedCaptions = this.stream.closedCaptions &&
        this.stream.closedCaptions.size > 0;
    const startTime = this.segmentReference.startTime + this.period.startTime;
    const endTime = this.segmentReference.endTime + this.period.startTime;
    this.mediaSourceEngine.appendBuffer(this.mediaState.type, data,
        startTime, endTime, hasClosedCaptions)
        .catch((err) => {
          shaka.log.v1('error reading chunk', err);
        });
  ...
  onReadUpdate(readObj) {
    if (readObj.done) {
      ...
      if (boxesInfo.found) {
        ...
        if (data) {
          this.onSegmentBoxReady(data);
        }
        this.offset_ = 0;
      } else {
        this.offset_ = boxesInfo.offset;
      }
    }
  }
