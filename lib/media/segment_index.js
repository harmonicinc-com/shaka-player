...
shaka.media.SegmentIndex = class {
  ...
  /**
   * It is only used on MetaSegmentIndex
   *
   * @param {number} refIdx
   * @return {number}
   * @private
   */
  findSegmentIndexNumberByRefIdx_(refIdx) {
    goog.asserts.assert(
        false, 'findSegmentIndexNumberByRefIdx() should not be ' +
      'used in SegmentIndex!');
    return -1;
  }
   ...
   * @export
   */
  dropStaleIndices(refIdx) {
    goog.asserts.assert(
        false, 'dropStaleIndices() should not be used in SegmentIndex!');
  }
  ...
  dropStaleReferences() {
    goog.asserts.assert(
        false, 'dropStaleReferences() should not be used in SegmentIndex!');
  }
...
shaka.media.MetaSegmentIndex = class extends shaka.media.SegmentIndex {
  ...
  count() {
    let sum = 0;
    for (const index of this.indexes_) {
      sum += index.numEvicted + index.references.length;
    }
    return sum;
  }

  /**
   * Find the position of the segment index by the segment reference index
   * returned by segmentIndex.find()
   *
   * @override
   * @param {number} refIdx Index returned by segmentIndex.find()
   * @returns {number} The position of the segment index
   * @private
   */
  findSegmentIndexNumberByRefIdx_(refIdx) {
    let sum = 0;
    for (const index of this.indexes_) {
      if (refIdx < sum + index.numEvicted + index.references.length) {
        return this.indexes_.indexOf(index);
      }
      sum += index.numEvicted + index.references.length;
    }
    return -1;
   ...
   */
  dropStaleIndices(refIdx) {
    const playingIndex = this.findSegmentIndexNumberByRefIdx_(refIdx);
    let dropIndex = -1;
    let lastEndTime = 0;
    console.log(JSON.stringify(this.indexes_));
    for (let i = 0; i < this.indexes_.length; i++) {
      const index = this.indexes_[i];
      // Reset evict count
      index.numEvicted = 0;
      for (let j = 0; j < index.references.length; j++) {
        const ref = index.references[j];
        // Drop if reference is non-continuous
        if (ref.startTime < lastEndTime) {
          dropIndex = i;
          break;
        } else {
          lastEndTime = ref.endTime;
        }
      }
      if (dropIndex !== -1) {
        break;
    ...
    if (playingIndex < dropIndex) {
      // Keep the first part
      // Drop all indices after drop index
      this.indexes_.length = dropIndex;
    } else {
      // Keep the second part
      // Drop all indices before drop index
      this.indexes_.splice(0, dropIndex);
    }
    console.log(JSON.stringify(this.indexes_));
  }

  /**
   * Scan all segment indices & references.
   * If there is discontinuity/jump backwards,
   * drop the reference & previous index
   *
   * New segment index will always be added at the end of the list, so it's
   * safe to drop previous ones.
   *
   * @override
   * @export
   */
