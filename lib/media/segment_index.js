...
shaka.media.SegmentIndex = class {
  ...
  constructor(references) {
    ...
    /** @private {?function():
     * (Array.<shaka.media.SegmentReference>|undefined)} */
   ...
   */
  update() {
    if (this.immutable_ || !this.updateCallback_) {
      return;
    }
    if (this.timer_) {
      this.timer_.stop();
    ...
    const references = this.updateCallback_();
    if (references && references.length > 0) {
      this.references.push(...references);
    } else {
      // No reference found. Retry after 0.5s
      this.timer_ = new shaka.util.Timer(() => {
        this.update();
      });
      this.timer_.tickAfter(0.5);
    }
  }
