...
shaka.media.MetaSegmentIndex = class extends shaka.media.SegmentIndex {
  ...
  /**
   * Scan all segment references.
   * If there is discontinuity/jump backwards, drop it
   *
   * @param {number} refIdx Indices earlier than this will be evicted.
   * e.g. We have 2 indices with length [2, 3]. If `refIdx` is 2, since
   * reference #2 belongs to the second segment index, the first index
   * will be evicted, while the second index will be kept.
   * @override
   * @export
   */
  dropStaleReferences(refIdx) {
    ...
    // Find the segment index number
    const segmentIndexNumber = this.findSegmentIndexNumberByRefIdx_(refIdx);
    if (segmentIndexNumber > 0) {
      // Drop all the indices before the segment index number
      this.indexes_.splice(0, segmentIndexNumber);
    }

    for (let j = 0; j < this.indexes_.length; j++) {
      const index = this.indexes_[j];
      // Reset evict count
      index.numEvicted = 0;
      for (let i = 0; i < index.references.length; i++) {
        const ref = index.references[i];
