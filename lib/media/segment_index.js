...
shaka.media.SegmentIndex = class {
  ...
  merge(references) {
    ...
    // New segment references should not contain evicted segments
    if (references.length > 0 && this.references.length > 0) {
      references = references.filter((r) => {
        return r.startTime >= this.references[0].startTime;
      });
    }
