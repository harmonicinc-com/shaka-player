...
shaka.media.PresentationTimeline = class {
  ...
  constructor(presentationStartTime, presentationDelay,
    ...
    /** @private {boolean} */
    this.enableLowLatency_ = false;
  }

  /**
   * @param {boolean} flag If true, enable load latency mode.
   * @export
   */
  setEnableLowLatency(flag) {
    this.enableLowLatency_ = flag;
  }
  ...
  getLiveEdge_() {
    ...
    // Date.now() is in milliseconds, from which we compute "now" in seconds.
    const now = (Date.now() + this.clockOffset_) / 1000.0;
    if (this.enableLowLatency_) {
      return Math.max(0, now - this.presentationStartTime_);
    }
    return Math.max(
        0, now - this.maxSegmentDuration_ - this.presentationStartTime_ - this.presentationDelay_);
  }
