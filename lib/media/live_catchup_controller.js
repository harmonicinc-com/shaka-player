...
shaka.media.LiveCatchUpController = class {
  ...
  constructor(playerInterface) {
    ...
    /** @private {?number} */
    this.targetLatencyOverride_ = null;

    /** @private {boolean} */
    this.syncPlayback_ = true;
  }
  ...
  updatePlayRate() {
    ...
    if (newPlayRate !== currentPlayRate && this.syncPlayback_) {
   ...
   */
  setTargetLatencyOverride(latency) {
    this.targetLatencyOverride_ = latency;
  }

  /**
   * @param {boolean} state
   */
  setSyncPlayback(state) {
    this.syncPlayback_ = state;
  }
  ...
  calculateNewPlaybackRate_() {
    ...
    let newRate = 1.0;
    const targetLatency = this.targetLatencyOverride_ ||
  ...
  /**
    ...
    */
  getLatency_() {
    return this.playerInterface_.getPresentationLatencyInfo().latency ||
      this.getTargetLatency_();
  }
};
