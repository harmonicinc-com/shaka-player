...
shaka.media.StreamingEngine = class {
  ...
  getProducerReferenceTimeInfo() {
    ...
    for (const [key, producerReferenceTime] of data.prtMap.entries()) {
      ...
      const wcaPrft = producerReferenceTime.wca_prft;
      const ptaPrft = producerReferenceTime.pta_prft;
      ...
      shaka.log.debug(`Getting ProducerReferenceTimeInfo for
          ...
          wca=${wca},
          wca from PRFT=${wcaPrft},
          pt=${pt},
          pta=${pta},
          pta from PRFT=${ptaPrft},
          type=${type}`);
  ...
  getPresentationLatencyInfo() {
    ...
    const pt = this.playerInterface_.getPresentationTime() * 1000;
    const {type, wca, pta} = latencyInfo;
    const wallClock = new Date(wca + (pt - pta));
    const latency = Math.round(Date.now() - wallClock.getTime()) / 1000;
    ...
    // NOTE: Do not include wallClock directly due to Closure restriction
    // Property key will be renamed randomly by Closure compiler
    data['wallClock'] = wallClock;
    return data;
  }
  ...
  parsePRFT_(reference, box) {
    ...
    prtData.pta_prft = Number(mediaTime) / prtData.timescale * 1000;

    shaka.log.v1(`parsed prft box 
