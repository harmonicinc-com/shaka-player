  ...
  getProducerReferenceTimeInfo() {
    ...
    for (const [key, producerReferenceTime] of data.prtMap.entries()) {
      ...
      const latency = Math.round(
          (wc - (wcaPrft || wca)) - (pt - (ptaPrft || pta)));
      const wcPrft = wcaPrft + (pt - ptaPrft);
      ...
      const data = {
        type, // Data source: "encoder" | "captured"
        latency,
        wca,
        wcaPrft, // undefined when unavailable
        pta,
        ptaPrft, // undefined when unavailable
        pt,
        wc,
        wcPrft, // NaN when unavailable
      };
      rv.push(data);
    }
    return rv;
