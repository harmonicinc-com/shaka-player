...
shaka.media.StreamingEngine = class {
    ...
    for (const [key, producerReferenceTime] of data.prtMap.entries()) {
      ...
      prtData.push({
        ...
        pta: ((hasPrft && ptaPrft !== undefined ? ptaPrft : pta) /
          producerReferenceTime.timescale * 1000) || 0,
        inband: hasPrft,
        clockOffset,
      });
    }
  ...
  getPresentationLatencyInfo() {
    ...
    const pt = this.playerInterface_.getPresentationTime() * 1000;
    const {type, wca, pta, clockOffset} = latencyInfo;
    const wallClock = new Date(wca + (pt - pta));
    const latency = Math.round(
        Date.now() - wallClock.getTime() + clockOffset) / 1000;
