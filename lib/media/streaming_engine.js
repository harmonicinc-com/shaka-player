...
shaka.media.StreamingEngine = class {
  ...
  switchInternal_(stream, clearBuffer, safeMargin, force) {
    ...
    /* number */
    const isDecreaseBandwidth =
        Number(mediaState.stream.bandwidth) < stream.bandwidth;
    ...
    this.makeAbortDecision_(mediaState, isDecreaseBandwidth).catch((error) => {
      if (this.playerInterface_) {
        goog.asserts.assert(error instanceof shaka.util.Error,
            'Wrong error type!');
        this.playerInterface_.onError(error);
   ...
   * @param {boolean} isDecreaseBandwidth
   * @private
   */
  async makeAbortDecision_(mediaState, isDecreaseBandwidth) {
    ...
    if ((this.config_.lowLatencyMode && isDecreaseBandwidth) ||
        this.shouldAbortCurrentRequest_(mediaState)) {
