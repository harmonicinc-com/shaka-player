...
shaka.media.StreamingEngine = class {
  ...
  getSegmentReferenceNeeded_(mediaState, presentationTime, bufferEnd) {
    ...
    if (mediaState.segmentIterator) {
      ...
      if (ref &&
        !(ref.startTime - 1 <= timeReq && ref.endTime > timeReq)) {
        shaka.log.debug(`Segment reference invalid. ` +
          `Reference startTime: ${ref.startTime}, endTime: ${ref.endTime};` +
          ` expected: ${timeReq}`);
        // Try to find the correct reference by time instead of index
        const refIdx = mediaState.stream.segmentIndex.find(timeReq);
        if (refIdx != null) {
          // We are able to find the correct reference.
          // Iterator went wrong, fix it
          ref = mediaState.stream.segmentIndex.get(refIdx);
          shaka.log.debug('Resetting segment iterator');
          mediaState.stream.segmentIndex.dropStaleReferences(refIdx);
          mediaState.segmentIterator = mediaState.stream.segmentIndex
              .getIteratorForTime(timeReq);
          mediaState.segmentIterator.next();
        }
        // return null if we couldn't find a valid reference
      }
      return ref;
