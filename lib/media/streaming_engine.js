...
shaka.media.StreamingEngine = class {
  ...
  getSegmentReferenceNeeded_(mediaState, presentationTime, bufferEnd) {
    ...
    if (mediaState.segmentIterator) {
      ...
      if (!ref || !(ref.startTime - 1 <= timeReq && ref.endTime > timeReq)) {
        shaka.log.v1('Segment reference null/invalid.');
        if (ref) {
          shaka.log.info(`Segment reference invalid. Reference startTime:` +
            ` ${ref.startTime}, endTime: ${ref.endTime}; expected startTime: ` +
            `${timeReq}`);
        }
        // Use find() to check if the iterator is corrupted (wrong count).
        const refIdx = mediaState.stream.segmentIndex.find(timeReq);
        if (refIdx != null) {
          // Iterator count went wrong, reset it
          shaka.log.info('Segment iterator corrupted. Resetting.');
          ref = mediaState.stream.segmentIndex.get(refIdx);
          mediaState.stream.segmentIndex.dropStaleIndices(refIdx);
          mediaState.segmentIterator = mediaState.stream.segmentIndex
              .getIteratorForTime(timeReq);
          mediaState.segmentIterator.next();
        }
      }
      // Will return null here if we couldn't find a valid reference
      // Could be end of stream/network error/not getting the latest manifest
      return ref;
