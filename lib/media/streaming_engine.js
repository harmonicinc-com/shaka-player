...
shaka.media.StreamingEngine = class {
  ...
  configure(config) {
    ...
    if (this.manifest_.presentationTimeline) {
      this.manifest_.presentationTimeline.setEnableLowLatency(
          this.config_.enableLowLatency);
    }
  ...
  */
  ...
  update_(mediaState) {
    ...
    if (this.playerInterface_.mediaSourceEngine.ended()) {
      return null;
    }
  ...
  getTimeNeeded_(mediaState, presentationTime) {
    ...
    let lastSegmentEnd = mediaState.lastSegmentReference.endTime;

    // Correct lastSegmentEnd by actual media buffer, in case segment duration
    // from manifest is inaccurate
    ...
    for (const i of buffered) {
      if (lastSegmentEnd > i.start && lastSegmentEnd < i.end) {
        lastSegmentEnd = i.end;
        break;
      }
    }
  ...
  async fetchAndAppend_(mediaState, presentationTime, reference) {
    ...
    try {
      ...
      if (this.config_.enableLowLatency) {
        // onSegmentAppendCompleted will be called by SegmentAppender
        return;
      }
   ...
   * @param {shaka.extern.Stream} stream
   */
  onSegmentAppendCompleted(mediaState, stream) {
    const StreamingEngine = shaka.media.StreamingEngine;
    const logPrefix = StreamingEngine.logPrefix_(mediaState);
    try {
      ...
      if (!this.playerInterface_ ||
          this.playerInterface_.mediaSourceEngine.ended()) {
        return;
      }
  ...
  async initSourceBuffer_(mediaState, reference) {
    ...
    const append = async () => {
      try {
        ...
        mediaState.initSegmentAppended = true;
      } catch (error) {
        mediaState.needInitSegment = true;
   ...
   * @param {number} wc
   * @param {number} pt
   * @param {shaka.extern.PresentationLatencyData} data
   * @return {Array.<Object>}
   */
  getPresentationLatency(wc, pt, data) {
    const ContentType = shaka.util.ManifestParserUtils.ContentType;
    ...
    for (const [key, producerReferenceTime] of data.prtMap.entries()) {
      ...
      const ts = producerReferenceTime.timescale;
      const type = producerReferenceTime.type;
      const latency = (wc - wca) - (pt - pta / ts);
      shaka.log.debug(`computing presentation latency for representation ` +
          `representationId=${repId}, wc=${wc}, wca=${wca}, pt=${pt} ` +
          `pta=${pta} ts=${ts} type=${type} latency=${latency}ms`);
  ...
  //   if (!this.manifest_.isInband) {
  //     return;
  //   }
  //   shaka.log.debug(`parsing prft box wca =
  //   ${this.manifest_.wca} pta = ${this.manifest_.pta}`);
  //   const version = box.reader.readUint8();
  //
  //   const flag1 = box.reader.readUint16();
  //   const flag2 = box.reader.readUint8();
  //   const flag = (flag1 << 8) + flag2;
  //   shaka.log.debug(`flag ${flag}`);
  //
  //   const referenceTrackId = box.reader.readUint32();
  //
  ...
  //   }
  //
  //   this.manifest_.wca = (new Date(ntpTimestampSec * 1000 +
  //       (new Date('1/1/1900')).getTime() -
  //       (new Date('1/1/1970')).getTime())).getTime();
  //   this.manifest_.pta = mediaTime;
  //
  //   shaka.log.debug(`parsed prft box wca =
  //    ${this.manifest_.wca} pta = ${this.manifest_.pta}`);
  // }
  ...
  async fetch_(mediaState, reference) {
    ...
    if (this.config_.enableLowLatency &&
        reference.constructor === shaka.media.SegmentReference) {
      const periodIndex = this.findPeriodContainingStream_(mediaState.stream);
      const currentPeriod = this.manifest_.periods[periodIndex];
      const segRef = /** @type {!shaka.media.SegmentReference} */(reference);
      request.customObj = new shaka.media.SegmentAppender(
          this.playerInterface_.mediaSourceEngine, mediaState,
          currentPeriod, segRef, mediaState.stream, this);
    }
 ...
 * @property {boolean} initSegmentAppended
 *   True after init segment is appended.
 */
shaka.media.StreamingEngine.MediaState_;
