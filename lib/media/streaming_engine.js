...
shaka.media.StreamingEngine = class {
  ...
  getPresentationLatency() {
    ...
    const tl = this.manifest_.presentationTimeline;
    const presentationTimeDrift = tl.getAutoCorrectedDrift() * 1000;
    let pt = this.playerInterface_.getPresentationTime() * 1000;
    pt -= presentationTimeDrift;
    const ContentType = shaka.util.ManifestParserUtils.ContentType;
    const repId = this.mediaStates_.get(ContentType.VIDEO).stream.originalId.split(",").pop();
    if (repId == null) {
  ...
  parsePRFT_(reference, box) {
    ...
    prtData.pta = Number(mediaTime) / prtData.timescale * 1000;
