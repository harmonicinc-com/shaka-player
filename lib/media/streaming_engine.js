...
shaka.media.StreamingEngine = class {
  ...
  async onUpdate_(mediaState) {
    ...
    try {
      const delay = await this.update_(mediaState);
      if (delay != null) {
   ...
   *
   * @param {shaka.media.StreamingEngine.MediaState_} mediaState
   * @return {Promise.<?number>} The number of seconds to wait until updating
   *  again or null if another update does not need to be scheduled.
   * @private
   */
  async update_(mediaState) {
    ...
    const bufferEnd =
        this.playerInterface_.mediaSourceEngine.bufferEnd(mediaState.type);
    const reference = await this.getSegmentReferenceNeeded_(
   ...
   * @return {Promise<shaka.media.SegmentReference>} The SegmentReference of the
   *   next segment needed. Returns null if a segment could not be found, does
   *   not exist, or is not available.
   * @private
   */
  async getSegmentReferenceNeeded_(mediaState, presentationTime, bufferEnd) {
    const logPrefix = shaka.media.StreamingEngine.logPrefix_(mediaState);
    goog.asserts.assert(
    ...
    if (mediaState.segmentIterator) {
      ...
      // appended.
      const timeReq = this.getTimeNeeded_(mediaState, presentationTime);
      const iter = mediaState.segmentIterator.current();
      // Check if the returned reference is what we need
      if (iter &&
        !(iter.startTime <= timeReq && iter.endTime > timeReq)) {
        shaka.log.warning('Segment reference invalid');
        // try switching variant
        await this.playerInterface_.refreshManifest();
        this.cancelUpdate_(mediaState);
        await this.abortOperations_(mediaState);

        // Assume the first variant
        ...
        // Refresh segment index and reset the iterator
        mediaState.segmentIterator = null;
      ...
      // shaka.log.debug(mediaState.stream.segmentIndex, refIdx, ref);
 ...
 *   refreshManifest: function():Promise
 * }}
 *
