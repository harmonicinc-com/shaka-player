...
shaka.media.StreamingEngine = class {
  ...
  constructor(manifest, playerInterface) {
    ...
    /**
     * Previous WCA from PRFT in case PRFT box is missing in the newest segment.
     * @type {number|undefined}
     * @private
     */
    this.previousWcaPrft_ = undefined;

    /**
     * Previous PTA from PRFT in case PRFT box is missing in the newest segment.
     * @type {number|undefined}
     * @private
     */
    this.previousPtaPrft_ = undefined;

    /**
     * Previous presentation start time.
     * @type {?number}
     * @private
     */
    this.lastPresentationStartTime_ = null;

    /**
     * Segment index refresh state.
     * Stores type of updated stream.
     * @type {Array.<string>}
     * @private
     */
    this.segmentIndexRefreshState_ = [];
  }
   ...
   */
  getLastSegmentRef(mediaType) {
    const ContentType = shaka.util.ManifestParserUtils.ContentType;
    let contentType = null;
    if (mediaType === ContentType.VIDEO) {
      contentType = ContentType.VIDEO;
    } else if (mediaType === ContentType.AUDIO) {
      contentType = ContentType.AUDIO;
    }

    if (contentType && this.mediaStates_.has(contentType)) {
      const stream = this.mediaStates_.get(contentType).stream;
      if (stream) {
        const segmentIndex = stream.segmentIndex;
        return segmentIndex.get(segmentIndex.count() - 1);
      }
    }
    return null;
  }
  ...
  async onUpdate_(mediaState) {
    ...
    const presentationStartTime = this.manifest_.presentationStartTime;
    if (this.lastPresentationStartTime_ &&
      this.lastPresentationStartTime_ !== presentationStartTime) {
      shaka.log.alwaysError('Availability start time changed');
      this.cancelUpdate_(mediaState);
      await this.abortOperations_(mediaState);
      let variant = this.manifest_.variants[0];
      if (this.playerInterface_.abrManager) {
        variant = this.playerInterface_.abrManager.chooseVariant() || variant;
      }
      this.switchVariant(variant, false, 0, true);
      const delta = this.playerInterface_.getPresentationTime() -
        presentationStartTime;
      const newPt = presentationStartTime + delta;
      // Regenerate segment indices
      mediaState.stream.segmentIndex = null;
      mediaState.lastSegmentReference = null;
      await mediaState.stream.createSegmentIndex();
      this.segmentIndexRefreshState_.push(mediaState.type);

      this.playerInterface_.createPlayhead(presentationStartTime + delta);
      shaka.log.debug(newPt);
      shaka.log.debug(mediaState.stream.segmentIndex);
    }
    if (this.segmentIndexRefreshState_.length > 0 &&
      !this.segmentIndexRefreshState_.includes(mediaState.type)) {
      // Regenerate segment indices
      mediaState.stream.segmentIndex = null;
      mediaState.lastSegmentReference = null;
      await mediaState.stream.createSegmentIndex();
      this.segmentIndexRefreshState_.push(mediaState.type);
    } else if (this.segmentIndexRefreshState_.length === 2) {
      this.segmentIndexRefreshState_ = [];
    }
    this.lastPresentationStartTime_ = presentationStartTime;
    ...
    if (!mediaState.stream.segmentIndex ||
      mediaState.stream.segmentIndex.count() === 0) {
      const thisStream = mediaState.stream;

      await mediaState.stream.createSegmentIndex();

      shaka.log.debug('segmentIndex: ', mediaState.stream.segmentIndex);
  ...
  /**
   * Get producer reference time info based on active video stream
   * @return {
   * Array.<shaka.media.StreamingEngine.ProducerReferenceTimeInfo>|null
   * }
   */
  getProducerReferenceTimeInfo() {
    ...
    const clockOffset = data.clockOffset;
    const pt = this.playerInterface_.getPresentationTime();
    const ContentType = shaka.util.ManifestParserUtils.ContentType;
    if (data.periodIdMap == null) {
      shaka.log.warning('getProducerReferenceTimeInfo cannot get periodIdMap');
      return null;
    }
    const {index, periodId} =
      this.getPeriodIdFromPresentationTime(pt, data.periodIdMap);
    const repId = this.mediaStates_.get(ContentType.VIDEO)
        .stream.originalId.split(',')[index];
    if (repId == null) {
      shaka.log.warning('getProducerReferenceTimeInfo cannot get video repId');
      return null;
    }
    ...
    for (const [key, producerReferenceTime] of data.prtMap.entries()) {
      ...
      const {wca, pta, pto,
        type, timescale, periodStart, isInBand} = producerReferenceTime;
      const wcaPrft = producerReferenceTime.wca_prft || this.previousWcaPrft_;
      const ptaPrft = producerReferenceTime.pta_prft || this.previousPtaPrft_;
      this.previousWcaPrft_ = wcaPrft;
      this.previousPtaPrft_ = ptaPrft;
      const hasPrft =
        wcaPrft !== undefined &&
        ptaPrft !== undefined &&
        wcaPrft >= 0 && ptaPrft >= 0;

      shaka.log.debug(`Getting ProducerReferenceTimeInfo for
          period ${periodId},
          representation ${repId}, 
          wcOffset=${clockOffset},
          wca=${wca},
          wca from PRFT=${producerReferenceTime.wca_prft},
          pt=${pt},
          pta=${pta},
          pta from PRFT=${producerReferenceTime.pta_prft},
          type=${type}`);

      prtData.push({
        type,
        wca: hasPrft && isInBand ? wcaPrft : wca,
        pta: hasPrft && isInBand ? ptaPrft : pta,
        pto,
        timescale,
        inband: hasPrft,
        clockOffset,
   ...
   * Available only if the manifest contains presentation latency.
   * If encoder latency is not available, fallback to captured latency
   *
   * Used in :
   * - Web Player SDK - Debug overlay
   * - SimpleLLAbrManager::increaseVideoBitrate
   * - LiveCatchUpController::getLatency
   *
   * @return {shaka.media.StreamingEngine.PresentationLatencyInfo|null}
   */
  getPresentationLatencyInfo() {
    const prftInfo = this.getProducerReferenceTimeInfo();
    if (!prftInfo) {
      return null;
    }

    const latencyInfoList = prftInfo
        .sort((a, b) =>
          ((a.inband === b.inband) ? 0 : a.inband ? -1 : 1))
        .sort((a, b) =>
          ((a.type === b.type) ? 0 : a.type === 'encoder' ? -1 : 1));

    if (latencyInfoList.length === 0) {
      return null;
    }

    const latencyInfo = latencyInfoList[0];
    const {type, wca, pta, pto,
      periodStart, timescale, clockOffset} = latencyInfo;
    const ptMs = this.playerInterface_.getPresentationTime() * 1000;
    const ptoMs = pto / timescale * 1000;
    const ptaMpdMs = periodStart * 1000 + (pta / timescale * 1000 - ptoMs);
    const wc = new Date(wca + (ptMs - ptaMpdMs));
    const latencyMs = Date.now() + clockOffset - wc.getTime();
    const latency = Math.round(latencyMs) / 1000;

    shaka.log.debug('PRFT info', Object.assign(latencyInfo, {
      ptMs,
      ptoMs,
      ptaMpdMs,
      wc,
    ...
    // NOTE: Some property key must be quoted due to Closure restriction
    // Property key will be renamed randomly by Closure compiler
    return {
      'type': type,
      'latency': latency,
      'wallClock': wc,
   ...
   * @return {{periodId: string, index: number}}
   */
  getPeriodIdFromPresentationTime(pt, periodIdMap) {
    ...
    return {
      index,
      periodId: periodIdMap.get(targetPt),
    };
  }

  /**
   * Parse PRFT box and add WCA and PTA as additional information.
   * @param {!shaka.media.SegmentReference} reference
   * @param {!shaka.extern.ParsedBox} box
   * @private
   */
  parsePRFT_(reference, box) {
    if (this.lastProcessedVideoSegment === reference.getUris()[0]) {
      shaka.log.v2(`skipping parsePRFT for parsed segment`);
      return;
    }
    ...
    const ntpTimestampSecMs = box.reader.readUint32() * 1000;
    const ntpTimestampFracMs = box.reader.readUint32() * 1000 / 2**32;
    const ntpTimestamp = ntpTimestampSecMs + ntpTimestampFracMs;
    let mediaTime;
    if (version === 0) {
    ...
    const {index, periodId} = this.getPeriodIdFromPresentationTime(
    ...
    prtData.wca_prft = wca;
    prtData.pta_prft = Number(mediaTime);

    shaka.log.debug(`parsed prft box 
         segment:${reference.getUris()}
         type ${type}
         ntpTimestampSecMs ${ntpTimestampSecMs}
         converted wca ${prtData.wca_prft}
         pto ${prtData.pto} 
         pta ${prtData.pta_prft} 
  ...
  async fetch_(mediaState, reference, streamDataCallback) {
    ...
    const isFullSpeed = this.isSegmentAtFullNetworkSpeed(reference.endTime);
    ...
    mediaState.operation = null;
    this.playerInterface_.onSegmentDownloadCompleted(Date.now() - startTime,
        response.data.byteLength, reference.getUris(), isFullSpeed);
   ...
   * @return {boolean}
   */
  isSegmentAtFullNetworkSpeed(segmentEndTime) {
    if (!segmentEndTime) {
      return false;
    }
    const timeline = this.manifest_.presentationTimeline;
    const start = timeline.getPresentationStartTime();
    if (start) {
      const liveEdge = Date.now() / 1000 - start;
      return segmentEndTime <= liveEdge;
    }
    return false;
  }
 ...
 * @export
 */
shaka.media.StreamingEngine.ProducerReferenceTimeInfo;
 ...
 * @export
 */
shaka.media.StreamingEngine.PresentationLatencyInfo;
 ...
 *   function(number, number, !Array.<string>, boolean),
 ...
 *   createPlayhead: function(!number):!shaka.media.Playhead,
 * }}
 *
