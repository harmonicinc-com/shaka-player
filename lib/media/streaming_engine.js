...
shaka.media.StreamingEngine = class {
   ...
   * Formerly named getPresentationLatency()
  ...
  getPresentationLatency() {
    ...
    // MP4 PRFT validation
    // If failed, fallback to MPD
    const hasPrft = prftInfo.some((prft) => prft.inband === true);
    const hasEncoder = prftInfo.some((prft) => prft.type === 'encoder');

    const latencies = prftInfo
        .filter((prft) => prft.inband === hasPrft && prft.type)
        .map((prft) => {
          const pt = this.playerInterface_.getPresentationTime() * 1000;
          const {type, wca, pta} = prft;
          const wallClock = new Date(wca + (pt - pta));

          let latency = Math.round((wallClock.getTime() - wca)
            - (pt - pta)) / 1000;

          if (prft.inband) {
            latency = Math.round(Date.now() - wallClock.getTime()) / 1000;
          }
          return {
            type,
            latency,
            wallClock: prft.inband ? wallClock : undefined,
          };
        });

    for (const el of latencies) {
      if (
        (hasEncoder && el.type === 'encoder') ||
        (!hasEncoder && el.type === 'captured')) {
        return {
          type: el.type,
          latency: el.latency,
